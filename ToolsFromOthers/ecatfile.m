...
% [ hd, message ]        = ecatfile( 'hd', fid, selmatrix );
   ...
   if strcmp( action, 'read' ) & isempty( message )
      ...
      % preallocate vol:
      if  file.mainheader.file_type ~= 5 % if not polarmap type
          switch descr.ftype
          case 'float32'
      ...
      else % if filetype is polarmap
            numofsegment = sum(hd.sh.sectors_per_ring);
            [ vol, count ] = fread( fidphys, numofsegment,'uint16');
            if count ~=  numofsegment
                message = 'file too short';
            end
      end
