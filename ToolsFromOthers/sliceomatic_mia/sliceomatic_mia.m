function sliceomatic_mia(p1,p2)
% sliceomatic_mia - Slice and isosurface volume exploration GUI
%
% sliceomatic_mia(DATA) - Use 3D double matrix DATA as a volume data
%
% Example:
%
...
%       sliceomatic_mia(v)
%
% Using sliceomatic_mia with no arguments is equivalent to the above
% example.
%
% Using the GUI:
% -------------
...
% ----
%
% 1) sliceomatic_mia does not use the `slice' command.  All slices are
...
% $$$     end
    ...
    if scaninfo.imfm(1) > RenderSize
		...
		hpbar = progbar(info);
        if scaninfo.float
            imaVOLr = (zeros(RenderSize,RenderSize,scaninfo.num_of_slice));
        else
 ...
 %set up the default X slice   
    ...
    set(newa,'buttondownfcn','sliceomatic_mia Xmove');
...
%set up the default Y slice
    ...
    setappdata(newa(2),'arrowslice',new);
    set(new,'alphadata',get(new,'cdata'),'alphadatamapping','scaled');
    set(newa,'buttondownfcn','sliceomatic_mia Ymove');
    set([new newa],'uicontextmenu',d.uic);
    d.draggedarrow=newa(2);
    dragprep(newa(2));
...
%set up the default Z slice
    ...
    setappdata(new,'controlarrow',newa);
    setappdata(newa(2),'arrowslice',new);
    set(newa,'buttondownfcn','sliceomatic_mia Zmove');
    set([new newa],'uicontextmenu',d.uic);
    d.draggedarrow=newa(2);
    dragprep(newa(2));
    setpointer(gcf,'SOM topbottom');
      ...
      switch p1
       ...
       case 'ISO'
        if strcmp(get(gcf,'selectiontype'),'normal')
          ...
          set(newa,'buttondownfcn','sliceomatic_mia ISOmove');
          % Make sure whatever buttonupfcn on the figure is run now to "turn
          % off" whatever was going on before we got our callback on the
          % arrow.
          buf = get(gcf,'windowbuttonupfcn');
       ...
       case 'print'
        ...
        close(newf);
       case 'close'%BL
        d=getappdata(gcf,'sliceomatic');%BL
        if d.UseGenericOpenGL%BL
            closereq;%BL
            feature('UseGenericOpenGL',1);%BL
        else%BL
            closereq;%BL
        end%BL
       otherwise
...
function dragprep(arrowtodrag)
  ...
  pushset(gcf,'windowbuttonupfcn','sliceomatic_mia up');
  pushset(gcf,'windowbuttonmotionfcn','sliceomatic_mia motion');
